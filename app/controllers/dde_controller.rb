...
class DdeController < ApplicationController
  ...
  def process_result
    ...
    json = params["person"] if params["person"].present? and json.blank?
    ...
    patient = CorePatient.find(patient_id) rescue nil

    if !patient.blank? and !session[:referrer].blank?
      session[:referrer] = session[:referrer] + "&ext_patient_id=#{patient_id}" if !session[:referrer].match("ext_patient_id")
    end
 
    print_and_redirect("/core_patient_registration/national_id_label?patient_id=#{patient_id}", session[:referrer]) and return if !patient.blank? and (json["print_barcode"] rescue false)

    route = session[:referrer]
    session.delete(:referrer)
    redirect_to route and return if !patient_id.blank?
  ...
  def process_data
    ...
    patient = CorePatientIdentifier.find_by_identifier(params[:id]).patient rescue nil
   
    national_id = ((patient.patient_identifiers.find_by_identifier_type(CorePatientIdentifierType.find_by_name("National id").id).identifier rescue nil) || params[:id])
    
    name = patient.person.names.last rescue nil
    
    address = patient.person.addresses.last rescue nil

    return_path = session[:referrer].blank? ? "http://#{request.host_with_port}/process_result" : session[:referrer]
    ...
    if patient.blank?
      redirect_to "/patient_not_found/#{national_id}" and return
    else
      process_result and return
    end
  ...
  def search_name
    ...
    mapped_params = @selected.join("=true&") + (params.length > 0 ? "=true" : '')
	
    redirect_to "/dde/search?#{mapped_params.gsub(/\s/, '%20')}" and return if params[:given_name].blank?
    result = {}
      
    json = {"results" => []}
  
    CorePerson.find(:all, :joins => [:names], :conditions => ["given_name = ? AND family_name = ? AND gender = ?", params["given_name"], params["family_name"], params["gender"]]).each do |person|
  ...
  def update_demographics
    ...
    patient = CorePerson.find(params[:person_id]).patient rescue nil
    ...
    national_id = patient.national_id || params[:id]
  ...
  def process_scan_data
    ...
    if JSON.parse(@results).length == 1
      ...
      if checked    
        ...
        patient = CorePatientIdentifier.find_by_identifier(@json["national_id"]).patient rescue nil
  ...
  def patient_not_found
    ...
    redirect_to session[:referrer] and return if !params[:create].blank? and params[:create] == "false"
  end
  ...
  def ajax_search
    ...
    CorePerson.find(:all, :joins => [:names], :limit => pagesize, :offset => offset, :conditions => ["given_name = ? AND family_name = ? AND gender = ?", params["given_name"], params["family_name"], params["gender"]]).each do |person|
