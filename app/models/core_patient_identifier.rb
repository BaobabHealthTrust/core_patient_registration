class CorePatientIdentifier < ActiveRecord::Base
  set_table_name "patient_identifier"
  set_primary_key :patient_identifier_id
  include CoreOpenmrs

  belongs_to :type, :class_name => "CorePatientIdentifierType", :foreign_key => :identifier_type, :conditions => {:retired => 0}
  belongs_to :patient, :class_name => "CorePatient", :foreign_key => :patient_id, :conditions => {:voided => 0}
